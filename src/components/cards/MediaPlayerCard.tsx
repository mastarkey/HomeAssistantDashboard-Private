...
const MediaPlayerCard: React.FC<MediaPlayerCardProps> = ({ entityId, entity }) => {
    ...
    <>
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className={`w-3 h-3 rounded-full flex-shrink-0 ${getStateColor()}`}></div>
        ...
        <button 
          ...
          className="text-gray-400 hover:text-white transition-colors opacity-0 group-hover:opacity-100 ml-2"
        >
      ...
      {!isOff && mediaTitle && (
        <div className="mb-4">
          <p className="text-sm text-gray-300 truncate font-medium">{mediaTitle}</p>
          {mediaArtist && (
            <p className="text-xs text-gray-500 truncate">{mediaArtist}</p>
          )}
          {mediaAlbum && (
            <p className="text-xs text-gray-600 truncate">{mediaAlbum}</p>
          )}
        </div>
      )}
      ...
      {!isOff && (
        <>
          {/* Playback Controls - only show if supported */}
          {supportsMediaControl && (
            ...
            {supportsPreviousNext && (
              <button
                ...
                className="p-2 rounded-full text-gray-400 hover:text-white hover:bg-gray-700 transition-all"
            ...
            <button
              ...
              className="p-3 rounded-full bg-gray-700 hover:bg-gray-600 transition-all disabled:opacity-50"
              disabled={isControlling}
            >
              {isPlaying ? (
